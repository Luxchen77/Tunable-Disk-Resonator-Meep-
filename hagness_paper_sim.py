...
for f_thz in [190]:
    ...
    norm_flux_region = mp.FluxRegion(center=mp.Vector3(wg_length/2 - 4, disk_radius + gap + wg_width/2),
                                    size=mp.Vector3(0, wg_width, 0))
    ...
    flux_region_bus = mp.FluxRegion(center=mp.Vector3(wg_length/2 - 4,
                                                    disk_radius + gap + wg_width/2),
                                    size=mp.Vector3(0, wg_width, 0))

    flux_region_drop = mp.FluxRegion(center=mp.Vector3(-wg_length/2 + 4,
                                                    -disk_radius - gap - wg_width/2),
                                    size=mp.Vector3(0, wg_width, 0))
    ...
    if rank == 0:
        ...
        folder_name = f"{datetime.now().strftime('%Y%m%d_%H%M')}_sim_norm_r{disk_radius}_g{gap}_f{f_thz}_res{resolution}_decay{field_decay}_hagness"
